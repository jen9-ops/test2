...
const append = (sender, text, cls) => {
  ...
  block.className = `msg ${cls}`;
  block.textContent = `${sender}: ${text}`;
...
/* ════════════  PROGRESS BADGE  ════════════ */
const TARGET = 1000;                 // 100 % = 1000/1000
...
async function trainFromURL() {
  ...
  try {
    ...
    const text = [...html.querySelectorAll('p')].map(p => p.textContent.trim()).join(' ');
    const s    = split(text);
    s.forEach(x => {
      knowledge.push({ request: x.toLowerCase(), answer: x });
      corpus.push(x);
...
/* Ctrl+Enter = send | Shift+Enter = newline (native) */
